...
export async function authorize(params: {
  ...
  if (tag.status !== 'ACTIVE') throw Object.assign(new Error('RFID tag is not active'), { statusCode: 422 });

  const vehicle = tag.vehicle;
  if (vehicle.status !== 'ACTIVE') throw Object.assign(new Error('Vehicle is suspended'), { statusCode: 422 });
  if (vehicle.allowedGrade !== grade) {
    throw Object.assign(
      new Error(`Misfuel: vehicle is authorized for ${vehicle.allowedGrade} only`),
      { statusCode: 422 }
    );
  }
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    // floor(available * 1000 / price) gives max ml we can afford
    ...
    if (maxMillilitres < MIN_FILL_ML) {
      throw Object.assign(new Error('Insufficient balance for minimum fill'), { statusCode: 422 });
    }
